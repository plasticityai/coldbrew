...
(
  ...
  if docker pull "registry.gitlab.com/plasticity/coldbrew/builder:$(python3 -c "import version; print(version.__version__)")"; then
      ...
      docker run --rm -it -v $(pwd)/customize:/BUILD/customize -v $(pwd)/${DIST_DIR}:/BUILD/${DIST_DIR} -v $(pwd)/installs/python-3.5.2/lib/libpython3.5:/BUILD/installs/python-3.5.2/lib/libpython3.5 -v $(pwd)/installs/python-3.5.2/lib/linked:/BUILD/installs/python-3.5.2/lib/linked -v $(pwd)/src:/BUILD/src -v $(pwd)/third_party:/BUILD/third_party coldbrew:latest /bin/bash -c "cd /usr/local/coldbrew/emsdk; source ./emsdk_env.sh; source /root/.cargo/env; cd /BUILD/; /bin/bash";
  else
